            ...
            if request.POST.get("check"):
                ...
                if value=="all-check":
                    ...
                    return render(request,"expensesReport.html",{
                        ...
                        "rental_expense_total":expensesData.objects.filter(date__range=[from_date,to_date]).filter(expenses_for='Rental').aggregate(Sum('paid_amount'))['paid_amount__sum'],
                        ...
                        checkNone(expensesData.objects.filter(date__range=[from_date,to_date]).filter(expenses_for='Rental').aggregate(Sum('paid_amount'))['paid_amount__sum']),
                        'from_date':from_date,
                        'to_date':to_date,
                    })
                    # return render(request, "revenue.html", {'revenueData':RentalData.objects.filter(rent_date__range=[from_date,to_date])})
