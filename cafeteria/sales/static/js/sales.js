...
function rowSelected(id) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#salesSubtotal').val(data[0]['order_total_price']);
            $('#salesDiscount').val(data[0]['order_total_discount']);
            $('#salesTotal').val(data[0]['order_total_price'] - data[0]['order_total_discount']);
            $('#salesDate').val(data[0]['order_date']);
            if (data[0]['customer_id'] != null) {
                $('#saleSCustomer').val(data[0]['customer_id']['customer_name']);
            }
            else {
