...
def addRental(request):
    try:
        ...
        payment= rentalPayment.objects.create(rent_amount=request.POST.get('rent-amount'), payment_mode=null_check(request.POST.get('payment-mode')),
                    ...
                    payment_status=request.POST.get('payment-status'), rental_id=add_rental,rent_pay_by=request.POST.get('rent-pay-from'),
...
def renew(request):
    try:
        ...
        payment.save()
        RentalData.objects.filter(id=request.POST.get('rent-id')).update(active_rent_id=payment.id)
    except Exception as e:
